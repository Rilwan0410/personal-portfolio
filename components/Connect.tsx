    ...
    >
      <div className="md:w-4/5 lg:w-3/4">
        <h6
          ...
          data-backdrop-text={contactData.mainData.title}
        >
          {contactData.mainData.title2}
        </h6>
        <h2 className="text-3xl lg:text-4xl font-poppins font-semibold mb-3 lg:mb-4 dark:text-white">
          {contactData.mainData.title3}
        </h2>
        {/* <p className="text-pColor dark:text-white/70">{contactData.mainData.description}</p> */}
      </div>
      <div className="xl:w-4/5 mt-6">
        <ul className="space-y-3">
          <li className="list-none inline-block px-4 py-2 border border-black/20 border-dashed rounded-full me-2 text-pColor hover:text-black transition ease-linear duration-100 dark:text-white/70 dark:border-white/20 dark:hover:text-white">
            Phone: {contactData.mainData.phone}
          </li>
          <li className="list-none inline-block px-4 py-2 border border-black/20 border-dashed rounded-full me-2 text-pColor hover:text-black transition ease-linear duration-100 dark:text-white/70 dark:border-white/20 dark:hover:text-white">
      ...
      <form
        method="post"
        id="contactform"
        className="space-y-8 mt-8 lg:mt-12"
      ...
      >
        ...
        />
        <textarea
          className="bg-transparent w-full h-[200px] border-b border-black/20 border-dashed pb-4 py-0 text-sm text-black placeholder:text-black/40 transition ease-out duration-[120] focus:outline-none focus:border-black dark:text-white dark:placeholder:text-white/50 dark:border-white/20 dark:focus:border-white"
          name="message"
          id="message"
          placeholder="Message"
          onChange={handleChange}
        ></textarea>
        <button
          className="inline-block bg-black text-white px-8 py-4 rounded-full font-mono text-sm transition linear duration-100 dark:bg-white/15 dark:hover:bg-white/20"
          type="submit"
        >
          Send Message
        </button>
      ...
      <div className="submit-result">
        {status === "success" && (
          <span
            id="success"
            className="transition-all duration-200 ease-out text-green-700"
          >
        ...
        {status === "error" && (
          <span
            id="error"
            className="transition-all duration-200 ease-out text-red-600"
          >
        ...
        {status === "loading" && (
          <span
            id="error"
            className="transition-all duration-200 ease-out ml-3 text-gray-200"
          >
            Sending...
          </span>
        )}
